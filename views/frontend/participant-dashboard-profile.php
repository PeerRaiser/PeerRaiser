        ...
        <div class="fundraisers">
            <?php while( $peerraiser['fundraisers']->have_posts() ) : $peerraiser['fundraisers']->the_post() ?>
                <div class="fundraiser">
                    ...
                    <div class="content">
                        <h3><?php the_title() ?></h3>
                        <p><?= get_the_title( $campaign_id ) ?></p>
                    </div>
                    <div class="controls">
                        <a href="<?= add_query_arg( 'edit', 'fundraiser', get_the_permalink() ) ?>"><?php _e( 'Manage', 'peerraiser' ); ?></a>
                        <a href="<?= get_the_permalink() ?>"><?php _e( 'View', 'peerraiser' ); ?></a>
                    </div>
                </div>
            <?php endwhile; ?>
        </div>
    ...
    <?php if ( $peerraiser['teams']->have_posts() ) : ?>
        ...
        <?php while ( $peerraiser['teams']->have_posts() ) : $peerraiser['teams']->the_post() ?>
            <div class="team">
                ...
                <div class="controls">
                    <a href="<?= add_query_arg( 'edit', 'team', get_the_permalink() ) ?>"><?php _e( 'Manage', 'peerraiser' ); ?></a>
                    <a href="<?= get_the_permalink() ?>"><?php _e( 'View', 'peerraiser' ); ?></a>
                </div>
            </div>
        <?php endwhile; ?>
